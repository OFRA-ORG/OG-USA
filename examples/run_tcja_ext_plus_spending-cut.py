...
def main(alpha, overwrite):
    ...
    reform_dir = os.path.join(CUR_DIR, "TCJA_Ext_Plus_Spending_Cut_"+str(alpha), "OUTPUT")
    ...
    if os.path.exists(reform_dir) and not overwrite:
        ...
        if alpha is not None:
            alpha = float(alpha)
            updated_params["alpha_bs_G"] = [(1-alpha), (1-alpha)**2, (1-alpha)**3, (1-alpha)**4]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", help="proportional spending cut compared to baseline spending, with 0.1 representing a 10-percent cut")
    parser.add_argument("--overwrite", help="Whether to overwrite existing run")
    args = parser.parse_args()
    main(args.alpha, args.overwrite)