...
def main(Z, overwrite):
    ...
    reform_dir = os.path.join(CUR_DIR, "TCJA_Ext_Plus_Prod_Gain_"+str(Z), "OUTPUT")
    ...
    if os.path.exists(reform_dir) and not overwrite:
        ...
        if Z is not None:
            Z = float(Z)
            updated_params["Z"] = [[Z], [Z**2], [Z**3], [Z**4], [Z**5], [Z**6], [Z**7], [Z**8], [Z**9], [Z**10]]
