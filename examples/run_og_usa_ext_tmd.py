...
def main(frisch = None, zeta_D = None, g_y_annual = None, tG1 = None):
    ...
    example_dir = "TCJA_TMD##frisch##" + str(frisch) + \
                  "##zeta_D##" + str(zeta_D) + \
    ...
    vis_folder = os.path.join(
        CUR_DIR, example_dir, "plots_and_tables"
    )

    if os.path.exists(vis_folder):
        print('Already did: ' + example_dir)
        ...
        ot.tp_output_dump_table(
            ...
            table_format="csv",
            path=os.path.join(
                vis_folder, "macro_time_series_output.csv",
            ),
        )
        ...
        ans.to_csv(
            os.path.join(
                vis_folder, "ogusa_example_output.csv"
            )
        )
